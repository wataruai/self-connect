...
async function apiCall(method, ...args) {
  try {
    ...
    if (data.error) throw new Error(data.message); return data.result;
...
createApp({
  data() {
    return {
      ...
      isLoading: false, isSending: false, isFetching: false, isChannelLoading: false, 
      isSendingTodo: false, isSendingEvent: false, isSendingLog: false, isSendingAdmin: false,
      errorMessage: null, showSettingsModal: false, showAddFriendModal: false, showCreateChannelModal: false, settingsForm: { display_name: '' }, searchIdInput: '', searchResult: null,
  ...
  methods: {
    ...
    verifyPin() { this.isLoading = true; setTimeout(() => { this.isLoading=false; if(this.pinInput === this.pendingLoginAccount.pin) { this.showPinModal = false; this.loginWithSaved(this.pendingLoginAccount); } else { this.errorMessage = "パスワードが違います"; this.pinInput=''; } }, 500); },
    ...
    async startApp() {
      ...
      try {
        ...
        if (res.error) { this.isLoading = false; return this.errorMessage = res.message; } 
        const userToSave = res.user; if(this.loginForm.pin) userToSave.pin = this.loginForm.pin; 
        this.saveToLocal(userToSave); this.loginWithSaved(userToSave); 
      } catch(e) { this.isLoading = false; this.errorMessage = e.message; }
    },
    async loginExisting() { 
      if (!this.existingSearchId) return; this.isLoading = true; 
      try {
        const r = await apiCall('searchUserBySearchId', this.existingSearchId);
        if (r.found) { this.saveToLocal(r.user); this.loginWithSaved(r.user); } else { this.isLoading = false; this.errorMessage = "見つかりません。"; }
      } catch(e) { this.isLoading = false; this.errorMessage = e.message; }
    },
    ...
    startPolling() { this.fetchData(false); this.pollingInterval = setInterval(()=>this.fetchData(false), 6000); },
    async fetchData(showLoading = false) {
      ...
      this.isFetching = true; if(showLoading) this.isChannelLoading = true;
      try {
        ...
        if(d.notifications && d.notifications.length > this.notifications.length) {
          const newNotifs = d.notifications.slice(0, d.notifications.length - this.notifications.length);
          newNotifs.forEach(n => this.pushNotify("新着通知", n.content));
        }
        if(d.messages && this.currentChannelId && d.messages.length > this.messages.length) {
          const last = d.messages[d.messages.length-1];
          if(last.user_id !== this.currentUser.user_id) this.pushNotify(last.user.display_name, last.content);
        }
        this.messages = d.messages||this.messages; this.gdms = d.gdms||this.gdms; this.friends = d.friends||this.friends; this.notifications = d.notifications||[]; this.tasks = d.tasks||this.tasks; this.tasks.forEach(t=>{if(t.answerInput===undefined) t.answerInput='';}); this.todos=d.todos||this.todos; this.events=d.events||this.events; 
        if(showLoading) this.scrollToBottom(); this.isFetching = false; this.isChannelLoading = false;
      } catch(e) { this.isFetching = false; this.isChannelLoading = false; }
    },
    ...
    async submitMessage(e, allowedStr) { 
      if (!this.inputText.trim() || this.isSending) return; this.isSending = true; 
      try {
        if(this.editMessageId) { await apiCall('updateMessage', this.editMessageId, this.currentUser.user_id, this.inputText); this.editMessageId = null; } 
        else { await apiCall('saveMessageData', this.currentChannelId, this.currentUser.user_id, this.inputText, allowedStr||''); }
    ...
    executeImageUpload() { 
      if(!this.uploadingImageFile) return; this.isSending = true; const reader = new FileReader(); 
      reader.onload = async (e) => { 
        try { const res = await apiCall('uploadFile', e.target.result.split(',')[1], this.uploadingImageFile.name, this.uploadingImageFile.type); if(res.success) { this.inputText += `\n${res.url}\n`; this.showImageUploadModal = false; this.submitMessage(null, this.selectedAllowedUsers.length>0 ? JSON.stringify(this.selectedAllowedUsers) : ''); } } catch(err) { this.isSending = false; this.errorMessage="UP失敗"; }
      }; reader.readAsDataURL(this.uploadingImageFile); 
    },
    ...
    async addTodo() { if(!this.newTodo) return; this.isSendingTodo = true; await apiCall('manageTodoEvent', 'add', 'todos', {userId:this.currentUser.user_id, content:this.newTodo}); this.newTodo=''; this.isSendingTodo = false; this.fetchData(); },
    ...
    async addEvent() { if(!this.newEvent.title||!this.newEvent.date) return; this.isSendingEvent = true; await apiCall('manageTodoEvent', 'add', 'events', {userId:this.currentUser.user_id, title:this.newEvent.title, date:this.newEvent.date}); this.newEvent={title:'',date:''}; this.isSendingEvent = false; this.fetchData(); },
    async deleteEvent(id) { await apiCall('manageTodoEvent', 'delete', 'events', {id:id}); this.fetchData(); },

    requestPushPermission() { if (window.Notification) Notification.requestPermission().then(p => this.errorMessage = `通知は ${p} になりました`); },
    testPush() { this.pushNotify("テスト通知", "これはセルフコネクトからのテストです。"); },
    ...
    async addStudyLog() { if(!this.studyForm.subject||!this.studyForm.duration) return; this.isSendingLog=true; await apiCall('addStudyLog', this.currentUser.user_id, this.studyForm.subject, this.studyForm.duration); this.studyForm={subject:'',duration:''}; this.isSendingLog=false; this.fetchData(); },
    ...
    async saveAdminUserEdit() { this.isSendingAdmin=true; try{ const r = await apiCall('updateUserByAdmin', this.currentUser.user_id, this.adminSelectedUser.user_id, this.adminEditForm); if(r.success) { this.showAdminUserModal = false; this.changeAdminTab('users'); } else { this.errorMessage = r.message; } }catch(e){} this.isSendingAdmin=false; },
    ...
    sendAdminNotification() { 
      this.isSendingAdmin = true; let fileP = Promise.resolve(''); 
      if(this.adminNotifFile) { fileP = new Promise(resolve => { const r=new FileReader(); r.onload=async ev=>{ try{ const res=await apiCall('uploadFile', ev.target.result.split(',')[1], this.adminNotifFile.name, this.adminNotifFile.type); resolve(res.success?res.url:''); }catch(e){resolve('');} }; r.readAsDataURL(this.adminNotifFile); }); } 
      fileP.then(async url => { await apiCall('sendNotification', this.currentUser.user_id, this.adminSelectedUser.user_id, this.adminNotifForm.content, url); this.adminNotifForm = {content: ''}; this.adminNotifFile=null; this.isSendingAdmin=false; this.errorMessage = "送信しました"; }); 
    },
    ...
    async createGroupDm() { this.isSending=true; this.gdmForm.members.push(this.currentUser.user_id); await apiCall('createGroupDm', this.gdmForm.name, this.gdmForm.members); this.showGroupDmModal=false; this.isSending=false; this.fetchData(); },
    ...
    async searchFriend() { this.isLoading=true; const r = await apiCall('searchUserBySearchId', this.searchIdInput); this.isLoading=false; if(r.found) this.searchResult = r.user; else this.errorMessage = "見つかりません"; },
    ...
    async saveSettings() { this.isLoading=true; const r = await apiCall('updateUserSettings', this.currentUser.user_id, this.settingsForm); this.isLoading=false; this.currentUser = r.user; this.showSettingsModal = false; },
  ...
  mounted() { 
    this.initAuth(); this.newTodo = ''; this.newEvent = {title:'', date:''}; 
